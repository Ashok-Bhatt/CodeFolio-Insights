...
const LinkCard = ({ platform, link, index, onSave, isUpdating, isAnyUpdating }) => {
    ...
    return (
        <div
            className={`group bg-white/90 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-gray-100 transition-all duration-500 transform relative overflow-hidden ${!isUpdating && isAnyUpdating ? 'opacity-70 grayscale-[0.5] pointer-events-none' : 'hover:shadow-2xl hover:-translate-y-2'}`}
        ...
        >
            ...
            {isUpdating && (
                <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-50 flex flex-col items-center justify-center animate-in fade-in duration-300">
                    ...
                    <p className="mt-3 text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] animate-pulse">
                        Updating ...
                    </p>
                </div>
            )}
                    ...
                    <div className="flex items-center gap-2 bg-gray-50/50 p-2.5 rounded-xl border border-gray-100">
                        ...
                        {isConnected && (
                            <button
                                ...
                                className="p-1.5 text-gray-400 hover:text-blue-500 transition-all"
                                disabled={isAnyUpdating}
                            >
                    ...
                    {isEditing ? (
                        <>
                            <button
                                onClick={handleSave}
                                disabled={isAnyUpdating}
                                className="flex-1 flex items-center justify-center gap-2 bg-green-500 text-white py-3.5 px-4 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-green-600 shadow-lg hover:shadow-green-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                            ...
                            <button
                                ...
                                disabled={isAnyUpdating}
                                className="flex items-center justify-center gap-2 bg-gray-100 text-gray-600 py-3.5 px-4 rounded-xl hover:bg-gray-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                <X className="h-4 w-4" />
                            </button>
                        </>
                        ...
                        <>
                            {isConnected ? (
                                <a
                                    ...
                                    className={`flex-1 flex items-center justify-center gap-2 bg-blue-600 text-white py-3.5 px-4 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg hover:shadow-blue-200 transition-all ${isAnyUpdating ? 'pointer-events-none opacity-50' : ''}`}
                                >
